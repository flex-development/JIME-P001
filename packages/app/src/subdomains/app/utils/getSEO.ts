import { stripTagsFromString } from '@baggie/string/convert/stripTagsFromString'
import { AnyObject, PartialOr } from '@flex-development/json/utils/types'
import {
  ICollectionListing,
  IMetafield,
  IPage,
...
import { EMPTY_SPACE } from '@flex-development/kustomzdesign/config/constants'
import { getProductImage } from '@flex-development/kustomzdesign/utils/getProductImage'
import { SEOProps } from '@subdomains/app/components/SEO'
import join from 'lodash/join'
import objectFromArray from './objectFromArray'

/**
 * @file Implementation - getSEO
 * @module subdomans/utils/getSEO
 */
...
const getSEO = (
  ...
  if (type === 'collection') {
    ...
    seo = {
      ...seo,
      description: stripTagsFromString(data.body_html, EMPTY_SPACE).trim(),
    ...
    // Get page metafields
    const page_metafields = objectFromArray(data.metafield, 'key')
    ...
    seo.description = stripTagsFromString(data.body, EMPTY_SPACE).trim()
