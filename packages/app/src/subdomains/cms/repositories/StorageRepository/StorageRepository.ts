 ...
 * @module subdomains/cms/repositories/StorageRepository/impl
 * @see https://tinacms.org/docs/media
 */

export default class StorageRepository implements MediaStore {
  /**
  ...
  async list({ directory: dir }: MediaListOptions = {}): Promise<MediaList> {
    ...
    await Promise.all(
      file_refs.map(async ref => {
        items.push(await StorageRepository.normalizeMedia(ref))
      })
    )
    ...
    await Promise.all(
      subfolder_refs.map(async ref => {
        items.push(await StorageRepository.normalizeMedia(ref, true))
      })
    )
  ...
  async uploadFile(media: MediaUploadOptions): Promise<Media> {
    ...
    return await StorageRepository.normalizeMedia(task.ref)
  }
}
