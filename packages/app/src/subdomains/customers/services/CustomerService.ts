 ...
 * @module subdomains/customers/services/CustomerService
 * @see https://shopify.dev/docs/admin-api/rest/reference/customers/customer
 */

export default class CustomerService
  extends QueryExecutor<ICustomer>
   ...
   */
  async find(query?: Query<ICustomer>): Promise<QEData<ICustomer>> {
  ...
  async findByEmail(email: ICustomer['email']): Promise<ICustomer | null> {
    const query: CustomerQuery = { email: { $eq: email } }
    const customers = (await this.find(query)) as Array<ICustomer>
  ...
  async get(id: ICustomer['id']): Promise<ICustomer> {
    const query: CustomerQuery = { id: { $eq: id } }
    const customers = (await this.find(query)) as Array<ICustomer>
