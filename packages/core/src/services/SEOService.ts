...
import objectFromArray from '../utils/objectFromArray'
...
class SEOService {
  ...
  static DEFAULT_IMAGE_PROPS = {
    height: 1920,
    src: `${process.env.KAPI_URL}/assets/images/morena.webp`,
    width: 1920
  }
  ...
  static async page(page: OrPromise<IPage>): OrNever<Promise<SEOData>> {
    ...
    } = objectFromArray<IObjectMetafield>(page.metafield, 'key')
  ...
  static async product(
    ...
    let seo: SEOData = {
      ...
      keywords: SEOService.formatKeywords(...listing.tags?.trim().split(',')),
    ...
    if (variants.length) {
      ...
      if (!variant_img) {
        const img = { ...SEOService.DEFAULT_IMAGE_PROPS, alt: title }
